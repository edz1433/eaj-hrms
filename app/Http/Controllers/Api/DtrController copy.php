...
class DtrController extends Controller
{
    ...
    public function syncDtr_(Request $request)
    {
        ...
        if (!empty($dates)) {
            ...
            // Fetch merged data for all dates
            try {
                $mergedData = DB::select("
                    SELECT 
                        id, emp_ID, date,
                        SUBSTRING_INDEX(CLEAN_DUPLICATES(
                            GROUP_CONCAT(NULLIF(time_in, '') ORDER BY time_in SEPARATOR ','), 
                            GROUP_CONCAT(NULLIF(device_id_in, '') ORDER BY device_id_in SEPARATOR ',')
                        ), '|', 1) AS cleaned_time_in,
                        SUBSTRING_INDEX(CLEAN_DUPLICATES(
                            GROUP_CONCAT(NULLIF(time_in, '') ORDER BY time_in SEPARATOR ','), 
                            GROUP_CONCAT(NULLIF(device_id_in, '') ORDER BY device_id_in SEPARATOR ',')
                        ), '|', -1) AS cleaned_device_id_in,
                        SUBSTRING_INDEX(CLEAN_DUPLICATES(
                            GROUP_CONCAT(NULLIF(time_out, '') ORDER BY time_out SEPARATOR ','), 
                            GROUP_CONCAT(NULLIF(device_id_out, '') ORDER BY device_id_out SEPARATOR ',')
                        ), '|', 1) AS cleaned_time_out,
                        SUBSTRING_INDEX(CLEAN_DUPLICATES(
                            GROUP_CONCAT(NULLIF(time_out, '') ORDER BY time_out SEPARATOR ','), 
                            GROUP_CONCAT(NULLIF(device_id_out, '') ORDER BY device_id_out SEPARATOR ',')
                        ), '|', -1) AS cleaned_device_id_out,
                        SUBSTRING_INDEX(CLEAN_DUPLICATES(
                            GROUP_CONCAT(NULLIF(time_over, '') ORDER BY time_over SEPARATOR ','), 
                            GROUP_CONCAT(NULLIF(device_id_over, '') ORDER BY device_id_over SEPARATOR ',')
                        ), '|', 1) AS cleaned_time_over,
                        SUBSTRING_INDEX(CLEAN_DUPLICATES(
                            GROUP_CONCAT(NULLIF(time_over, '') ORDER BY time_over SEPARATOR ','), 
                            GROUP_CONCAT(NULLIF(device_id_over, '') ORDER BY device_id_over SEPARATOR ',')
                        ), '|', -1) AS cleaned_device_id_over
                    FROM dtrs
                    WHERE date IN ('" . implode("','", $dateList) . "')
            ...
            foreach ($mergedData as $data) {
                $updates[] = [
                    'id' => $data->id,
                    'device_id_in' => $data->cleaned_device_id_in ?: null,
                    'device_id_out' => $data->cleaned_device_id_out ?: null,
                    'device_id_over' => $data->cleaned_device_id_over ?: null,
                    'time_in' => $data->cleaned_time_in ?: null,
                    'time_out' => $data->cleaned_time_out ?: null,
                    'time_over' => $data->cleaned_time_over ?: null,
                ];
            }
