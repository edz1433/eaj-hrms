    ...
    public function syncDtr(Request $request)
    {
        ...
        if (!empty($dates)) {
            ...
            $mergedData = DtrTest::select(
                ...
                'date',
                DB::raw("MAX(id) as id"),
                DB::raw("GROUP_CONCAT(device_id_in ORDER BY time_in SEPARATOR ',') AS device_id_in"),
                DB::raw("GROUP_CONCAT(device_id_out ORDER BY time_out SEPARATOR ',') AS device_id_out"),
                DB::raw("GROUP_CONCAT(device_id_over ORDER BY time_over SEPARATOR ',') AS device_id_over"),
                DB::raw("GROUP_CONCAT(time_in ORDER BY time_in SEPARATOR ',') AS time_in"),
                DB::raw("GROUP_CONCAT(time_out ORDER BY time_out SEPARATOR ',') AS time_out"),
                DB::raw("GROUP_CONCAT(time_over ORDER BY time_over SEPARATOR ',') AS time_over")
            )
            ->whereIn('date', $dateList)
            ->groupBy('emp_ID', 'date')
            ->get();
