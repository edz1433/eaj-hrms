...
class LoginAuth
{
    ...
    public function handle(Request $request, Closure $next)
    {
        if (auth()->guard('web')->check()) {
            ...
            $employeeStatus = \App\Models\Status::find(auth()->guard('employee')->user()->emp_status);
            if ($request->is('leave') && strtolower((string) optional($employeeStatus)->status_name) !== 'regular') {
                return redirect()->route('empPDS')->with('error1', 'You do not have permission to access this page');
            }
        }else {
