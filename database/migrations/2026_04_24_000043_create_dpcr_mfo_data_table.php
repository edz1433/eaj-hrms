...
return new class extends Migration
{
    ...
    {
        Schema::create('dpcr_mfo_data', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('dpcr_mfo_id')->index();
            $table->unsignedBigInteger('opcr_mfo_data_id')->nullable()->index();
            ...
            $table->tinyInteger('lock')->default(0);       // 1=locked by reviewer
            $table->timestamps();

            $table->foreign('dpcr_mfo_id')->references('id')->on('dpcr_mfos')->onDelete('cascade');
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('dpcr_mfo_data');
    }
};
