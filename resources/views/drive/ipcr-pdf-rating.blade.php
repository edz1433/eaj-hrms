            ...
            <thead>
                ...
                <tbody>
                    ...
                    @foreach($cores as $core)
                        ...
                        @php
                            $filteredIpcrMfoDatas = in_array($cat, [1, 2])
                                ? $datas->where('ipcr_mfo_id', $core->id)->where('category', $cat)
                                : $datas->where('ipcr_mfo_id', $core->id);
                        @endphp
                    ...
                    @foreach($strats as $strat)
                        ...
                        @php
                            $filteredIpcrMfoDatas = in_array($cat, [1, 2])
                                ? $datas->where('ipcr_mfo_id', $strat->id)->where('category', $cat)
                                : $datas->where('ipcr_mfo_id', $strat->id);
                        @endphp
                    ...
                    @foreach($supports as $supp)
                        ...
                        @php
                            $filteredIpcrMfoDatas = in_array($cat, [1, 2])
                                ? $datas->where('ipcr_mfo_id', $supp->id)->where('category', $cat)
                                : $datas->where('ipcr_mfo_id', $supp->id);
                        @endphp
...
<script type="text/php">
    if (isset($pdf)) {
        $pdf->page_script('
            ...
            $footer_text = "Doc Control Code: CPSU-F-HRMO-12-REV01        Effective Date: 08/07/2024        Page No.:" . $PAGE_NUM . " of " . $total_pages;
