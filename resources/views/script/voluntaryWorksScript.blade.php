<script>
    $(document).on('click', '.voluntaryworks_delete', function(e){
    var id = $(this).val();
    var url = "{{ route('voluntaryworkDelete', ['id' => ':id']) }}";
        ...
        if (result.isConfirmed){
            $.ajax({
                ...
                success: function (response) {  
                    $(".voluntaryworks-row.row-" + id).fadeOut(2000);
                    Swal.fire({
                    title:'Deleted!',
...
$(document).on('click', '.voluntaryworks_approve', function(e) {
    var id = $(this).val();
    var url = "{{ route('voluntaryworksApprove', ['id' => ':id']) }}";
    url = url.replace(':id', id);
